...
public class RegisterDoctor implements Task {
	...
	public RegisterDoctor(List<RegisterForm> registerForm) {
	...
	public <T extends Actor> void performAs(T actor) {
		...
		actor.attemptsTo(Enter.theValue(registerForm.get(0).getIdDoctorNumber().trim()).into(HerokuappRegisterPage.ID_NUMBER));
		
		actor.attemptsTo(Click.on(HerokuappRegisterPage.SAVE));
	}

	public static RegisterDoctor inHerokuappPage(List<RegisterForm> data) {
		
		return instrumented(RegisterDoctor.class, data);
	}
